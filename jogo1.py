...
TEXTO_COMPLETO = (
    ...
    "Projeto: tal\n"
    "Professor orientador: André de Moraes\n"
    "Instituição: Intituto Federal de Santa Catarina - Câmpus Garopaba")

linhas_de_texto = TEXTO_COMPLETO.split('\n')
superficies_de_texto = [FONT.render(linha, True, preto) for linha in linhas_de_texto]
velocidade_rolagem = 0.09
            ...
            if mode == "menu":
                ...
                if botao_voltar_rect.collidepoint(event.pos):
                    if som_agudo1_fase1.get_num_channels() > 0 or som_grave1_fase1.get_num_channels() > 0:  
                        ...
                        som_grave1_fase1.stop(); click_sound.play(); mode = "menu_fase1"
                    else:
                        click_sound.play(); mode = "menu_fase1"
                elif botao_avancar_rect.collidepoint(event.pos):
                    if som_agudo1_fase1.get_num_channels() > 0 or som_grave1_fase1.get_num_channels() > 0:  
                        som_agudo1_fase1.stop()
                        som_grave1_fase1.stop(); click_sound.play(); mode = "pergunta_fase1"
                    else:
                        click_sound.play(); mode = "pergunta_fase1"
                ...
                elif botao_fase1_rect.collidepoint(event.pos):click_sound.play(); mode = "fase1"
